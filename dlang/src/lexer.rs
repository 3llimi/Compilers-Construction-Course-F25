    ...
    fn lex_number(&mut self, first: char) -> Token {
        ...
        while let Some(c) = self.peek() {
            if c.is_ascii_digit() {
                ...
                // Проверяем, что после точки идёт цифра (для Real числа)
                // Если после точки НЕ цифра, это не часть числа
                if let Some(&next_ch) = self.input.get(self.pos + 1) {
                    if next_ch.is_ascii_digit() {
                        // Это вещественное число
                        s.push(self.advance().unwrap()); // поглощаем '.'
                        is_real = true;
                        
                        // Поглощаем дробную часть
                        ...
                        // После точки не цифра - НЕ вещественное число
                        // Останавливаемся, точку обработает парсер как Member access
                        break;
                    }
                } else {
                    // Конец ввода
                ...
                break; // выходим после обработки вещественной части
            } else {
                break;
            }
        }
